...
export function calculateFinancials(input: FullFinancialInput): CalculatedFinancialResults {
  ...
  // Revenue Coverage Score (0 to 10)
  ...
  if (monthlyProfit <= 0) {
    riskIndicators.push({
      ...
      description: `Projected monthly deficit. Revenue is insufficient to cover total fixed and variable costs.`,
    });
    ...
    riskIndicators.push({
      ...
      description: `Profit margin of ${profitMargin}% leaves low buffer for market fluctuations.`,
    });
  }
  ...
  if (paybackPeriodMonths === null || paybackPeriodMonths > 36) {
    riskIndicators.push({
      ...
      description: paybackPeriodMonths === null
        ? 'Capital cannot be recovered with current negative or zero monthly profits.'
        : `Payback requires ${paybackPeriodMonths} months, increasing capital lock-in.`,
    });
  }
  ...
  if (breakEvenCapacityPercentage !== null && breakEvenCapacityPercentage > 85) {
    riskIndicators.push({
      ...
      description: `Requires ${breakEvenCapacityPercentage}% of expected monthly volume just to break even.`,
    });
  }
...
export const calculateClientFinancials = calculateFinancials;
