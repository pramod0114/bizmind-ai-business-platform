/**
 * BizMind – Client-Side Financial Feasibility Calculation Engine
 * 
 * Instant live preview calculator mirroring the backend calculation service.
 */
...
function sanitizeNonNegative(val: unknown, fallback = 0): number {
...
export function calculateClientFinancials(
  ...
  if (sellingPrice > 0) {
    ...
    if (contributionMarginPerUnit <= 0) {
      ...
      breakEvenMessage = `Break-even reached at ${breakEvenUnits} units / ₹${breakEvenRevenue.toLocaleString('en-IN')} monthly revenue.`;
    }
    ...
    breakEvenCalculable = false;
    breakEvenMessage = 'Break-even cannot be reached under current assumptions: Enter valid revenue assumptions.';
  }
  ...
  const scenarios = {
    conservative: computeScenario('Conservative', scenarioConfig.conservative.revenueAdjustment, scenarioConfig.conservative.expenseAdjustment),
    expected: computeScenario('Expected', 0, 0),
    optimistic: computeScenario('Optimistic', scenarioConfig.optimistic.revenueAdjustment, scenarioConfig.optimistic.expenseAdjustment),
  };
  ...
  // 10. Financial Feasibility Evaluation
  ...
  if (monthlyProfit > 0) {
    if (profitMargin >= 30) {
      ...
      feasibilityScore += 22;
      reasons.push(`Moderate profit margin of ${profitMargin}%; operations are profitable.`);
    } else {
      feasibilityScore += 12;
      reasons.push(`Low profit margin of ${profitMargin}%; vulnerable to minor cost increases.`);
    }
  } else {
  ...
  if (annualRoi !== null) {
    if (annualRoi >= 75) {
      ...
      feasibilityScore += 8;
      reasons.push(`Low annual ROI of ${annualRoi}%.`);
    } else {
      reasons.push(`Negative annual ROI (${annualRoi}%).`);
    }
  } else if (totalInitialInvestment === 0 && monthlyProfit > 0) {
    feasibilityScore += 25;
    reasons.push('Zero initial capital outlay with immediate net positive cash flow.');
  }
  ...
  if (paybackPeriod !== null) {
    if (paybackPeriod <= 12) {
      ...
      feasibilityScore += 12;
      reasons.push(`Payback period of ${paybackPeriod} months (~3 years).`);
    } else {
      feasibilityScore += 5;
      reasons.push(`Extended payback period of ${paybackPeriod} months (>3 years).`);
    }
  } else {
  ...
  if (breakEvenCapacityPercentage !== null) {
    if (breakEvenCapacityPercentage <= 50) {
      ...
      feasibilityScore += 5;
      reasons.push(`High break-even barrier requiring ${breakEvenCapacityPercentage}% of volume to cover costs.`);
    } else {
      reasons.push(`Break-even requires ${breakEvenCapacityPercentage}% of volume (exceeds 100% capacity).`);
    }
  }
  ...
  if (monthlyProfit < 0) {
    ...
    riskIndicators.push({
      area: 'Profit Margin',
      severity: 'MEDIUM',
      reason: `Thin profit margin of ${profitMargin}%. A minor 5–10% dip in revenue or supplier cost increase could eliminate profits.`,
    });
  }
  ...
  if (paybackPeriod === null) {
    ...
    riskIndicators.push({
      area: 'Payback Period',
      severity: 'MEDIUM',
      reason: `Payback window is ${paybackPeriod} months (>3 years). Long payback horizon increases risk from market changes.`,
    });
  }
  ...
  if (monthlyRevenue > 0 && totalMonthlyFixedExpenses / monthlyRevenue > 0.7) {
    riskIndicators.push({
      ...
      severity: 'HIGH',
      reason: `Fixed overhead accounts for ${round2((totalMonthlyFixedExpenses / monthlyRevenue) * 100)}% of monthly revenue.`,
    });
  }
  ...
  if (breakEvenCapacityPercentage !== null && breakEvenCapacityPercentage > 85) {
    riskIndicators.push({
      ...
      reason: `Business must maintain at least ${breakEvenCapacityPercentage}% of sales volume just to avoid operational losses.`,
    });
  }
  ...
  if (targets.targetPaybackPeriod !== undefined && targets.targetPaybackPeriod > 0) {
    ...
    const diff = targets.targetPaybackPeriod - pPayback;
    targetComparisons.push({
